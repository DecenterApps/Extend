...
def callback(ch, method, properties, body):
    ...
    response = contract.call().getUsernameForAddress(from_address)

    if response == '0x0':
        from_username = 'anonymous user'
    else:
        from_username = response

    if 'months' in decoded_body:
        messenger.send(to_username=to_username, from_username=from_username, months=decoded_body['months'])
    else:
        ...
        if not response:
            ...
            messenger.send(to_username=to_username, from_username=from_username, eth_amount=val)
